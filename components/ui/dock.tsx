  ...
  return (
    ...
    >
      <motion.div
        ...
        className={cn(
          "fixed top-4 left-1/2 z-50 transform -translate-x-1/2 flex items-start justify-center h-14 p-2 gap-3  bg-opacity-90 rounded-xl",
...
function DockCardInner({ src, label, id, children }: DockCardInnerProps) {
  ...
  const isActive = animatingIndexes.includes(parseInt(id))

  return (
    <span className="relative flex justify-center items-center z-0 overflow-hidden w-full h-full">
      ...
      {/* Handle images with AnimatePresence */}
      {src ? (
        <AnimatePresence mode="popLayout">
          {(!isActive || !children) ? (
            <motion.img
              layoutId={`img-${id}`} // Unique identifier for layout animations
        ...
        <span
          className={cn(
            "relative z-0 flex items-center justify-center w-full h-full text-sm font-medium tracking-wide transition-colors duration-200",
            isActive ? "text-black" : "text-black/60 hover:text-black"
          )}
        >
          <PixelParagraph text={label} pixelWords={[label]} as="span" />
        </span>
      ) : null}
    </span>
  )
...
function DockCard({ children, id }: DockCardProps) {
  ...
  const isActive = dock.animatingIndexes.includes(parseInt(id))
  ...
  useEffect(() => {
    if (isActive) {
      opacity.set(1)
      controls.start({
        y: 24, // Move the card down by 24 pixels
        transition: {
  ...
  }, [isActive, controls, opacity])

  // Handle click event to start the card's animation exclusively
  const handleClick = () => {
    dock.setAnimatingIndexes([parseInt(id)])
  }
  ...
  let widthSync = useTransform(distance, [-150, 0, 150], [90, 130, 90])
